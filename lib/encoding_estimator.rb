...
module EncodingEstimator
  ...
  def EncodingEstimator.ensure_utf8(
          data,
          languages       = [ :de, :en ],
    ...
    EncodingEstimator.detect(
        data, languages, encodings, directions, include_default
    ).result.perform( data )
  end
  ...
  def EncodingEstimator.detect(
    ...
    Detector.new(
        Conversion.generate( encodings, directions, include_default ), languages
    ).detect data, 8
  end
end